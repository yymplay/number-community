...
function change2(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(document.getElementById(s[i]+"2").selectedIndex-1));
	};
	
	var ss=document.getElementById(s[v]+"2");
	
	with(ss){
		length = 0;
		options[0]=new Option(opt0[v],opt0[v]);
		
		if(v && document.getElementById(s[v-1]+"2").selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
...
function _init_area2(){  //初始化函数
	...
	for(i=0;i<s.length-1;i++){
	  document.getElementById(s[i]+"2").onchange=new Function("change2("+(i+1)+")");
	}
    change2(0);
